...
class ProcessPhase1ResultsServiceTest {
  ...
  @Test
  void executeProcessesPhase1Results() throws Exception {
    AnalysisRequestEntity parent =
        AnalysisRequestEntity.builder().analysisId("ana-1").analysisStatus("PROCESSING").build();
    when(analysisRepo.findById("ana-1")).thenReturn(Mono.just(parent));
    doNothing().when(batchOrchestrator).processPhase1Results("ana-1", "gs://bucket/out1/");

    ProcessRequest req = new ProcessRequest();
    req.setAnalysisId("ana-1");
    req.setGcsUri("gs://bucket/out1/");

    StepVerifier.create(service.execute(req))
        .assertNext(
            resp -> {
              assertThat(resp.getStatusCode()).isEqualTo(HttpStatus.OK);
              assertThat(resp.getBody()).contains("Phase 1 batch prediction results successfully processed");
            })
        .verifyComplete();

    verify(batchOrchestrator).processPhase1Results("ana-1", "gs://bucket/out1/");
  }
}
