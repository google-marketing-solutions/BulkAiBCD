...
class CheckPhase1StatusServiceTest {
  ...
  private CheckPhase1StatusService service;
  ...
  void setUp() {
    ...
    service = new CheckPhase1StatusService(analysisRepo, cloudTasksClient, vertexBatchClient);
  }
  ...
  void executeReEnqueuesWhenJobRunning() throws Exception {
    ...
    when(vertexBatchClient.getBatchPredictionJob("job-1")).thenReturn(job);
    ...
    verify(cloudTasksClient)
        .enqueueTask(eq("/api/v2/worker/check-phase1-status"), anyString(), eq("ana-1_P1_POLL_attempt_2"), eq(600));
  }
  ...
  void executeEnqueuesProcessResultsWhenJobSucceeded() throws Exception {
        ...
        BatchPredictionJob.newBuilder()
            ...
            .setOutputInfo(
                BatchPredictionJob.OutputInfo.newBuilder()
                    .setGcsOutputDirectory("gs://bucket/out/")
                    .build())
            .build();
    when(vertexBatchClient.getBatchPredictionJob("job-1")).thenReturn(job);
    ...
    verify(cloudTasksClient)
        .enqueueTask(eq("/api/v2/worker/process-phase1-results"), anyString(), eq("ana-1_P1_PROCESS_RESULTS"), any());
  }
  ...
  void executeEnqueuesFallbackWhenJobFailed() throws Exception {
    ...
    BatchPredictionJob job =
        BatchPredictionJob.newBuilder()
            ...
            .setError(com.google.rpc.Status.newBuilder().setMessage("Resource exhausted").build())
            .build();
    when(vertexBatchClient.getBatchPredictionJob("job-1")).thenReturn(job);
    ...
    verify(cloudTasksClient)
        .enqueueTask(eq("/api/v2/worker/process-phase1-results"), anyString(), eq("ana-1_P1_PROCESS_RESULTS_FAIL"), any());
  }
}
